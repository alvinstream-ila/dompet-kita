...
const MonthlyDonutChart = dynamic(
  () =>
    import('@/components/charts/MonthlyDonutChart').then(
      (m) => m.MonthlyDonutChart
    ),
  {
    ssr: false,
    loading: () => (
...
export const HomeBudgeting: React.FC = () => {
  return (
    <div className="mt-6 grid grid-cols-1 gap-4 md:col-span-1 md:gap-8 lg:col-span-9 lg:mt-0 lg:grid-cols-12">
      <Card className="group glass-card flex flex-col p-5 sm:p-6 md:col-span-1 md:p-10 lg:col-span-5">
        <CardHeader className="mb-8 flex flex-row items-center justify-between p-0">
          <div>
            <CardTitle className="mb-1 text-base font-black tracking-tighter text-slate-800 uppercase md:text-lg">
              Budgeting <span className="text-blue-royal">Tracker</span>
            </CardTitle>
            <p className="text-[10px] font-black tracking-[0.2em] text-slate-400 uppercase">
              Distribusi harta kita ❤️
            </p>
          </div>
          <div className="bg-blue-royal/10 group-hover:bg-blue-royal/20 rounded-2xl p-3 transition-colors">
            <div className="bg-blue-royal h-1.5 w-1.5 animate-ping rounded-full" />
          </div>
        </CardHeader>
        ...
        <div className="mt-8 rounded-[24px] border border-white/40 bg-white/30 p-4 text-center shadow-inner backdrop-blur-md">
