...
const INSECURE_PATTERNS = [
	...
	{
		pattern: /https:\/\/gateway\.storjshare\.io\/[^\s"']*/,
		label: "Public Storj URL (no expiry)",
	},
	// Hardcoded receipt or upload paths
	{
		pattern: /\/storage\/receipts\/[^\s"']+/,
		label: "Direct storage path (not signed)",
	},
	// Supabase storage public bucket
	{
		pattern: /supabase\.co\/storage\/v1\/object\/public\//,
		label: "Supabase public bucket URL",
	},
];
...
function scanFile(filePath) {
	...
	INSECURE_PATTERNS.forEach(({ pattern, label }) => {
		...
		if (match) {
			...
			console.log(
				`   👉 Fix: Use backend endpoint /api/storage/signed-url?key=... instead\n`,
			);
			issues++;
		}
	});
...
if (issues === 0) {
	...
	console.error(
		`❌ Found ${issues} insecure asset reference(s). Please fix before deploying.`,
	);
	process.exit(1);
}
