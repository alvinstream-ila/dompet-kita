...
const glassKeywords = ['backdrop-filter', 'blur', 'border-opacity', '--glass-opacity'];
let glassScore = 0;
...
if (glassScore >= 3) {
    console.log("✅ SUCCESS: Glassmorphism tokens detected (Score: " + glassScore + "/4)");
} else {
    console.warn("⚠️  WARNING: Glassmorphism tokens are missing or weak! Design might look 'Flat'.");
}
...
if (cssContent.includes('hsl(') || cssContent.includes('oklch(')) {
    ...
    console.error("❌ CRITICAL: Found HEX/Generic Colors! Standard lowered to MVP level.");
    process.exit(1);
}
