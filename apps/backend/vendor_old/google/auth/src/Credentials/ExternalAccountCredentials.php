 ...
 */
class ExternalAccountCredentials implements FetchAuthTokenInterface, GetQuotaProjectInterface, GetUniverseDomainInterface, ProjectIdProviderInterface, UpdateMetadataInterface
{
    ...
    private static function buildCredentialSource(array $jsonKey): ExternalAccountCredentialSourceInterface
    {
        ...
        if (
            ...
            && preg_match('/^aws(\d+)$/', $credentialSource['environment_id'], $matches) === 1
        ) {
            if ($matches[1] !== '1') {
                throw new InvalidArgumentException(
