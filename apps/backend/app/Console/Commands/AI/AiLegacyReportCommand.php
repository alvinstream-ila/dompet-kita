...
class AiLegacyReportCommand extends Command
{
    ...
    public function handle(GenerateReportAction $generateReportAction): int
    {
        try {
            ...
            $this->warn('Aset Terdata (Total: Rp '.number_format((float) ($report['data']['financial_summary']['total_assets'] ?? 0), 0, ',', '.').'):');
            /** @var array<int, array<string, mixed>> $assetDetails */
            $assetDetails = $report['data']['asset_details'];
            $this->table(['Nama Aset', 'Nilai'], $assetDetails);

            $this->warn('Utang Aktif (Total: Rp '.number_format((float) ($report['data']['financial_summary']['total_loans'] ?? 0), 0, ',', '.').'):');
            /** @var array<int, array<string, mixed>> $activeLoans */
            $activeLoans = $report['data']['active_loans'];
            $this->table(['Debitur', 'Nilai', 'Jatuh Tempo'], $activeLoans);
