...
class CheckLegacyInactivity extends Command
{
        ...
        foreach ($users as $user) {
            ...
            if ($user->legacy_grace_start_at) {
                ...
                if ($daysElapsed >= 7) {
                    ...
                    $user->notify(new LegacyGracePeriodNotification($user, 7 - $daysElapsed));
                }
    ...
    protected function triggerFinalLegacy(User $user, GenerateReportAction $generateReportAction): void
    {
        ...
        try {
            ...
            $filename = $generateReportAction->execute($user);

            // 2. Prepare report data for notification
            $reportData = [
                'filename' => $filename,
                'financial_summary' => [
                    'total_assets' => (float) $user->assets()->sum('value'),
                    'total_loans' => (float) $user->loans()->where('is_paid', false)->sum('amount'),
                ],
                'vault_url' => config('app.frontend_url').'/legacy-vault/claim?token='.bin2hex(random_bytes(16)), // Placeholder for claim flow
            ];

            // 3. Notify Partner (Internal or External)
            if ($user->partner) {
                $user->partner->notify(new LegacyTriggerNotification($user, $reportData));
