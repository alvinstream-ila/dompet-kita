...
class CfoForecast extends Command
{
     ...
     * @var string
     */
    protected $signature = 'cfo:forecast {--months=12 : Number of months to forecast} {--user=}';
    ...
    public function handle(ForecastWealthAction $action): int
    {
        ...
        try {
            ...
            if (! $user) {
                $this->error($userId ? "User with ID {$userId} not found." : 'User ID required via --user flag.');
                $status = 1;
            } else {
                $data = $action->execute($user, $months);
